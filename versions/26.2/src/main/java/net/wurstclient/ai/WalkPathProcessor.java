...
public class WalkPathProcessor extends PathProcessor
{
	...
	public void process()
	{
		...
		if(pos.getX() != nextPos.getX() || pos.getZ() != nextPos.getZ())
		{
			...
			if(pos.getY() < nextPos.getY())
			{
				...
				}else if(WURST.getHax().spiderHack.isEnabled())
				{
					BlockPos wall = SpiderPathPlanner.findWall(pos,
						candidate -> BlockUtils.getState(candidate)
							.blocksMotion());
					if(wall != null)
					{
						WURST.getRotationFaker().faceVectorClientIgnorePitch(
							Vec3.atCenterOf(wall));
						MC.options.keyUp.setDown(true);
					}else
						jumpUp(nextPos);
				}else
	...
	private void jumpUp(PathPos nextPos)
	{
