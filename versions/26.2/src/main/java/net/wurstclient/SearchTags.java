 ...
 * search bar in the ClickGUI.
 *
 * <p>
 * ClickGUI can already find features by their name and description, so
 * repeating that information in the search tags is pointless. However, names
 * and descriptions of settings are not used by ClickGUI, so repeating those
 * can make sense if people are likely to search for them.
 *
 * <p>
 * ClickGUI is not case-sensitive, so for example "NukerLegit" and "nukerlegit"
 * are treated the same. Search still preserves spaces and word order, so for
 * example "NukerLegit", "Nuker Legit" and "Legit Nuker" are all
 * treated differently.
 *
 * <p>
