...
public class CleanUpScreen extends Screen
{
	...
	public void init()
	{
		...
		addRenderableWidget(cleanUpButton = new CleanUpButton(width / 2 - 100,
			height / 4 + 144 + 12, () -> "清理",
			"Start the 清理 with the settings\n" + "you specified above.\n"
		...
		addRenderableWidget(new CleanUpButton(width / 2 - 100, height / 4 + 0 + 12,
	...
	private void saveServerList()
	{
		...
		ServerSelectionList serverListSelector =
			((IMultiplayerScreen)prevScreen).getServerListSelector();
		
		serverListSelector.setSelected(null);
		serverListSelector.updateOnlineServers(prevScreen.getServers());
	}
		...
		int mouseY)
	{
		...
		{
			if(!button.isHoveredOrFocused() || !(button instanceof CleanUpButton))
				continue;
	...
	private final class CleanUpButton extends Button
	{
			...
			int mouseY, float partialTicks)
		{
			...
			extractDefaultLabel(context.textRendererForWidget(this,
