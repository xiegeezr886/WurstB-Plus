...
public final class CreatingTemplateState extends TemplateToolState
{
	...
	private final ArrayDeque<BlockPos> last1024Added = new ArrayDeque<>();
	...
	public void onEnter(TemplateToolHack hack)
	{
		...
		sortingHelper = new TreeSet<>(Comparator
			.<BlockPos> comparingDouble(pos -> pos.distSqr(origin))
	...
	public void onUpdate(TemplateToolHack hack)
	{
		...
		for(int i = 0; i < blocksPerTick && !sortingHelper.isEmpty(); i++)
		{
			...
			{
				double dPos = last1024Added.getLast().distSqr(pos);
				if(dPos >= dCurrent)
					continue;
		...
		float partialTicks)
	{
		...
		AABB bounds = BoxBackports.enclosing(start, end).deflate(1 / 16.0);
		...
		List<AABB> boxes = last1024Added.stream()
