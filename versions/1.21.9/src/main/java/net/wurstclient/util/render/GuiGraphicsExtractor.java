...
public final class GuiGraphicsExtractor
{
	...
	{
		// Forge's 1.21.9 GuiGraphics exposes getRenderState() publicly, but
		// keeps the guiRenderState field private. Mod access transformers are
		// not applied to the compile classpath in FG7, so use the getter
		// instead of the field (the field is only reachable via the AT at
		// runtime).
