...
public final class NoChatReportsOtf extends OtherFeature
	implements UpdateListener, ChatInputListener, WorldChangeListener
{
	...
	public void onUpdate()
	{
		...
		try
		{
			Field chatSessionField = ClientPacketListener.class.getDeclaredField("chatSession");
			chatSessionField.setAccessible(true);
			Field encoderField = ClientPacketListener.class.getDeclaredField("signedMessageEncoder");
			encoderField.setAccessible(true);
			
			if(isActive())
			{
				chatSessionField.set(netHandler, null);
				encoderField.set(netHandler, SignedMessageChain.Encoder.UNSIGNED);
				
			}else if(chatSessionField.get(netHandler) == null)
				MC.getProfileKeyPairManager().prepareKeyPair()
					.thenAcceptAsync(optional -> optional
						.ifPresent(profileKeys -> {
							try
							{
								chatSessionField.set(netHandler,
