	...
	implements UpdateListener, PacketInputListener, RenderListener
{
	...
	private void processSoundUpdate(PacketInputEvent event)
	{
		...
		// check sound type
		if(!SoundEvents.FISHING_BOBBER_SPLASH
	...
	{
		// check packet type
		if(!(event.getPacket() instanceof ClientboundSetEntityDataPacket update))
			return;
		
		// check if the entity is a bobber
		if(!(MC.level
			.getEntity(update.id()) instanceof FishingHook bobber))
			return;
