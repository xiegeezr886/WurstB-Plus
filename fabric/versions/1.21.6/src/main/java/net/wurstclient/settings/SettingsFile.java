...
public final class SettingsFile
{
	...
	private void loadSettings(Feature feature, JsonObject json)
	{
		...
		for(Entry<String, JsonElement> e : json.entrySet())
		{
			...
			if(setting == null)
				setting = settings.values().stream()
					.filter(candidate -> candidate.matchesName(key)).findFirst()
					.orElse(null);
			...
			try
			{
				setting.fromJson(e.getValue());
			}catch(RuntimeException exception)
			{
				System.err.println("Couldn't load setting " + feature.getName()
					+ "." + setting.getName());
