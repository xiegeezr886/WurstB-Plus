...
public enum RenderUtils
{
	 ...
	 *
	 * <p>We intentionally avoid the {@code GuiGraphicsExtractorAccessor}
	 * mixin here: on 26.1.2-Fabric the accessor interface can end up loaded
	 * before its target class is transformed (e.g. with the game language set
	 * to zh_cn), which makes Mixin throw an IllegalClassLoadError when the
	 * interface is loaded from game code ("mixin has not been applied"),
	 * crashing the client as soon as any custom GUI geometry is rendered
	 * (Radar, checkboxes, lines, ...). Reading the field via reflection skips
	 * the accessor class-load check entirely and is equally fast after the
	 * initial lookup.
	 */
	...
	{
		return WurstClient.MC.gameRenderer.getMainCamera().position();
	}
		...
		Vec3 end, int color, boolean depthTest)
	{
		...
		Vec3 offset = getCameraPos().reverse();
		drawLine(matrices, buffer, start,
		...
		int color, boolean depthTest)
	{
		...
		drawCurvedLine(matrices, buffer,
			points2, color);
		vcp.endBatch(layer);
	}
		...
		boolean depthTest)
	{
		...
		RenderType layer = WurstRenderLayers.getLines(depthTest);
		VertexConsumer buffer = vcp.getBuffer(layer);
		drawOutlinedBox(matrices, buffer,
			box.move(getCameraPos().reverse()), color);
		vcp.endBatch(layer);
	}
