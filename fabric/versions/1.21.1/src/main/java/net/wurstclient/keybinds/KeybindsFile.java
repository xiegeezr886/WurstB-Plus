...
public final class KeybindsFile
{
	...
	private Set<Keybind> parseFile(Path path) throws IOException, JsonException
	{
		...
		for(Entry<String, String> entry : wson.getAllStrings().entrySet())
		{
			...
			String commands = migrateGuiBinding(keyName, entry.getValue());
	...
	static String migrateGuiBinding(String keyName, String commands)
	{
		if("key.keyboard.right.shift".equals(keyName)
			&& "clickgui".equalsIgnoreCase(commands.trim()))
			return "navigator";
