...
public enum InventoryUtils
{
	...
	{
		return selectItem(stack -> stack.is(item), maxInvSlot,
			takeFromOffhand);
	}
	...
	public static boolean selectItem(int slot)
	{
		...
		// otherwise, swap with the currently selected item
			...
			im.windowClick_SWAP(toNetworkSlot(slot), inventory.getSelectedSlot());
	...
	public static void setCreativeStack(int slot, ItemStack stack)
	{
		...
		MC.player.connection.send(
			new ServerboundSetCreativeModeSlotPacket(toNetworkSlot(slot), stack));
	}
}
