...
| `baritone-api-forge-1.20.1.jar` | 构建和 JarJar 打包所需的本地 Baritone Forge API |
...
| `hacks/KeepSprintHack.java` + `mixin/PlayerMixin.java` + `util/KeepSprintPolicy.java` | 向前移动每 Tick 维持疾跑；攻击时原位保留 100% 水平速度并阻止原版取消疾跑；策略移出 Mixin，避免 Mixin 0.8.5 方法可见性错误 | LiquidBounce/FDP Mixin 思路，按 Forge 1.20.1 Mojmap 重写 |
...
公开仓库只保留可构建主项目，不包含开发期间使用的第三方源码、反编译目录、参考压缩包或可执行文件。Wurst、WurstCN、BleachHack、Meteor、LiquidBounce、FDPClient、RavenBS++、Aristois/EMC、FrogClient、LabyMod、Sinka、Xaero Minimap 与 JourneyMap 仅作为架构和行为研究来源；活动实现统一按 Forge 1.20.1、Mojmap 和 Java 17 重写。

GPL/MIT 或其他许可资源应保留来源与许可证声明；无许可证反编译内容仅用于行为核对，不直接复制。随包字体与资源许可证位于 `src/main/resources/META-INF/licenses/`。
...
项目版本为 `1.5.0`。Forge 47.4.10、Java 17、Gradle 8.11。50 个测试文件中的 133 项测试、完整构建、重混淆、JarJar 打包、关键 Mixin 方法可见性及 Mixin 包隔离检查通过；69 个 Mixin 已写入发布包。KeepSprint 的 `PlayerMixin` 已改用原生 `@Redirect`，发布包不再引用运行时生成的 Mixin `Args` 类。发布产物为 `build/libs/WurstB+ Plus-v1.5.0-MC1.20.1-all.jar`（29,094,464 bytes），SHA-256 为 `0B70C7DB27FBBA84D7801F7BEA0E74022043C49D67E92A2818E762C77AE9CED1`。
