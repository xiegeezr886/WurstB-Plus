...
该仓库目录只包含可构建的 Forge 主项目、测试、资源、Gradle Wrapper、许可证和构建所需的 Baritone API，不包含开发期间使用的反编译目录、第三方客户端源码、参考压缩包、日志、运行实例、账号配置或代理脚本。

项目在实现过程中参考了 Wurst、WurstCN、BleachHack、Meteor、LiquidBounce、FDPClient、RavenBS++、Aristois/EMC、FrogClient、LabyMod、Sinka、Xaero Minimap 与 JourneyMap 的公开架构和交互思路。活动代码统一按 Forge 1.20.1、Mojmap 和 Java 17 重写；无许可证的反编译内容仅用于行为核对，不直接复制代码或资源。随包字体和第三方资源的归属及许可证保存在 `src/main/resources/META-INF/licenses/`。
