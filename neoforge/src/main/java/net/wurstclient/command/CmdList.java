...
public final class CmdList
{
	...
	public void registerAddonCommand(Command cmd)
	{
		if(hasCommand(cmd.getName()))
			throw new IllegalArgumentException(
				"Command already registered: " + cmd.getName());
		cmds.put(cmd.getName(), cmd);
	}
	...
	private final List<BrigadierCommand> brigadierCommands = new ArrayList<>();

	public void registerBrigadierCommand(BrigadierCommand cmd)
	{
		if(hasBrigadierCommand(cmd.getName()))
			throw new IllegalArgumentException(
				"Brigadier command already registered: " + cmd.getName());
		brigadierCommands.add(cmd);
	}

	public boolean hasBrigadierCommand(String name)
	{
		return brigadierCommands.stream()
			.anyMatch(cmd -> cmd.getName().equalsIgnoreCase(name));
	}

	public void buildBrigadierDispatcher(
		CommandDispatcher<CommandSourceStack> dispatcher)
	{
		for(BrigadierCommand cmd : brigadierCommands)
