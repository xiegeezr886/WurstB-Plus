...
public enum RotationUtils
{
	...
	public static double getAngleToLastReportedLookVec(Rotation rotation)
	{
		...
		Rotation lastReported = player.isPassenger()
			...
			: new Rotation(((net.wurstclient.mixin.LocalPlayerAccessor)(Object)player).getYRotLast(), ((net.wurstclient.mixin.LocalPlayerAccessor)(Object)player).getXRotLast());
	...
	{
		Rotation start = new Rotation(MC.player.yRotO, MC.player.xRotO);
		return slowlyTurnTowards(start, end, maxChange);
	}

	public static Rotation slowlyTurnTowards(Rotation start, Rotation end,
		float maxChange)
	{
		...
		float pitchChange =
			Math.abs(Mth.wrapDegrees(endPitch - startPitch));
	...
	/**
	 * Smoothly turns towards the end rotation using the specified smoothing
	 * algorithm.
	 */
	public static Rotation smoothTurnTowards(Rotation end, float maxChange,
		RotationSmoothing mode)
	{
		Rotation start = new Rotation(MC.player.yRotO, MC.player.xRotO);
		return RotationSmoothing.smooth(start, end, maxChange, mode);
	}
