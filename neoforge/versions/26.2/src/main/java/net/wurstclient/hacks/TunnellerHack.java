	...
	implements UpdateListener, RenderListener
{
	...
	private void updateCyanBuffer()
	{
		...
		vertexBuffers[0] = EasyVertexBuffer.createAndUpload(PrimitiveTopology.LINES,
	...
	public ArrayList<BlockPos> getAllInBox(BlockPos from, BlockPos to)
	{
		...
		int toLeft =
			to.getX() * left.getStepX() + to.getZ() * left.getStepZ();
		...
		public void run()
		{
			...
			if(!boxes.isEmpty())
				vertexBuffers[1] = EasyVertexBuffer.createAndUpload(
					PrimitiveTopology.LINES, DefaultVertexFormat.POSITION_COLOR_NORMAL, buffer -> {
						for(AABB box : boxes)
	...
	private class FillInFloorTask extends Task
	{
		...
		public boolean canRun()
		{
			...
			for(BlockPos pos : BlockUtils.getAllInBox(from, to))
				if(!BlockUtils.getState(pos).isCollisionShapeFullBlock(MC.level, pos))
					blocks.add(pos);
	...
	private class PlaceTorchTask extends Task
	{
		...
		public boolean canRun()
		{
			...
			int yellow = 0x80FFFF00;
			vertexBuffers[4] = EasyVertexBuffer.createAndUpload(PrimitiveTopology.LINES,
				DefaultVertexFormat.POSITION_COLOR_NORMAL, buffer -> {
	...
	private void placeBlockSimple(BlockPos pos)
	{
		...
		for(int i = 0; i < sides.length; i++)
		{
			...
			if(MC.level.clipWithInteractionOverride(eyesPos, hitVecs[i], neighbor,
