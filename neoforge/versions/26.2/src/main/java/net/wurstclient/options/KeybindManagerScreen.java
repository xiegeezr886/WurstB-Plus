...
public final class KeybindManagerScreen extends Screen
{
	...
	public void init()
	{
		...
		addRenderableWidget(addButton = Button
			.builder(Component.literal("添加"),
				b -> minecraft.setScreenAndShow(new KeybindEditorScreen(this)))
		...
		addRenderableWidget(backButton = Button
			.builder(Component.literal("返回"), b -> minecraft.setScreenAndShow(prevScreen))
			.bounds(width / 2 + 2, height - 28, 100, 20).build());
		
		addRenderableWidget(Button.builder(Component.literal("重置按键"),
			b -> minecraft.setScreenAndShow(new ConfirmScreen(confirmed -> {
				if(confirmed)
					WurstClient.INSTANCE.getKeybinds()
						.setKeybinds(KeybindList.DEFAULT_KEYBINDS);
				minecraft.setScreenAndShow(this);
			}, Component.literal("确定要重置按键绑定？"),
				Component.literal("无法撤销！"))))
			.bounds(8, 8, 100, 20).build());
		
		addRenderableWidget(Button
			.builder(Component.literal("配置..."),
				b -> minecraft.setScreenAndShow(new KeybindProfilesScreen(this)))
	...
	private void edit()
	{
		...
		minecraft.setScreenAndShow(new KeybindEditorScreen(this, keybind.getKey(),
			keybind.getCommands()));
	}
