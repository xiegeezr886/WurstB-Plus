	...
	implements PacketInputListener, UpdateListener
{
	...
	{
		Vec3 incoming = new Vec3(packet.getXa(), packet.getYa(),
			packet.getZa());
		if(!shouldApply())
			return;
	...
	private void handleExplosionVelocity(ClientboundExplodePacket packet)
	{
		...
		Vec3 oldKnockback = packet.playerKnockback().orElse(Vec3.ZERO);
		Vec3 newKnockback = new Vec3(oldKnockback.x * horizontalMultiplier,
			oldKnockback.y * verticalMultiplier,
			oldKnockback.z * horizontalMultiplier);
		ClientboundExplodePacketMixin accessor =
			(ClientboundExplodePacketMixin)(Object)packet;
		accessor.wurst_setPlayerKnockback(java.util.Optional.of(newKnockback));
	}
