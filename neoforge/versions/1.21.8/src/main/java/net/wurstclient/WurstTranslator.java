...
public class WurstTranslator implements ResourceManagerReloadListener
{
		...
		Iterable<String> langCodes, BiConsumer<String, String> entryConsumer)
	{
		...
		{
			loadTranslationFile(manager, "translations/" + langCode + ".json",
				langCode, entryConsumer);
			if("zh_cn".equals(langCode))
				loadTranslationFile(manager,
					"translations/zh_cn_names.json", "zh_cn feature names",
					entryConsumer);
		}
	}
