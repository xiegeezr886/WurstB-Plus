...
public final class GuiGraphicsExtractor
{
	...
	public void requestCursor(Object type)
	{
		// CursorType does not exist before 1.21.9, no-op
	}
		...
		boolean focus)
	{
		...
		// 1.21.6 has no textHighlight() at all, so draw the highlight box.
		inner.fill(RenderPipelines.GUI_TEXT_HIGHLIGHT, x, y, x + width,
			y + height, 0xFF0000FF);
		if(focus)
	...
	public void extractDeferredElements(int mouseX, int mouseY,
		float partialTicks)
	{
		// 1.21.6 only defers tooltips, not arbitrary GUI elements.
		inner.renderDeferredTooltip();
	}
	
	public GuiRenderState getRenderState()
	{
		return inner.guiRenderState;
	}
