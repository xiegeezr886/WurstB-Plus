...
public final class ClickGuiScreen extends Screen
{
		...
		double dragY)
	{
		...
		double mouseY = context.y();
		int button = context.button();
		if(button != InputConstants.MOUSE_BUTTON_LEFT || draggedWindow == null)
		...
		double deltaY)
	{
		...
		{
			gui.handleMouseScroll(mouseX, mouseY, deltaY);
			return true;
		}
		for(int i = windows.size() - 1; i >= 0; i--)
			if(windows.get(i).scroll(mouseX, mouseY, deltaY))
				return true;
	...
	private final class CategoryWindow extends TemplateWindow
	{
		...
		@Override
		protected void renderRow(GuiGraphicsExtractor graphics, int index, int rowY,
			boolean hovering)
		{
			...
			int button)
		{
			...
			if(button == InputConstants.MOUSE_BUTTON_LEFT && mouseX >= badgeLeft
	...
	private final class ConfigWindow extends RowsWindow
	{
		...
		protected List<PanelRow> rows()
		{
			...
			rows.add(actionRow("打开配置文件夹", () -> false, () -> {
				...
				Blaze3D.openPath(WURST.getSettingsProfileFolder());
			}));
			return rows;
		}
		...
		private void saveSelected()
		{
			if(selectedProfile == null)
			{
				MC.setScreenAndShow(new EnterProfileNameScreen(ClickGuiScreen.this,
					this::createProfile));
				return;
			}
	...
	private final class FontWindow extends RowsWindow
	{
		...
		protected List<PanelRow> rows()
		{
			...
			rows.add(actionRow("打开字体文件夹", () -> false, () -> {
				...
				Blaze3D.openPath(WURST.getGuiPreferences().getFontsFolder());
			}));
			return rows;
		}
		...
		protected List<PanelRow> rows()
		{
			...
			return List.of(
				...
				actionRow("HUD编辑器", () -> false,
					() -> MC.setScreenAndShow(new net.wurstclient.hud2.HudEditorScreen())));
		}
	}
