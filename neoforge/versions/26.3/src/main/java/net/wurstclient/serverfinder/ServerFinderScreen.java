...
public class ServerFinderScreen extends Screen
{
	...
	public void init()
	{
		addRenderableWidget(searchButton =
			Button.builder(Component.literal("搜索"), b -> searchOrCancel())
		...
		addRenderableWidget(
			Button
				.builder(Component.literal("教程"),
					b -> Blaze3D.openUri(URI.create("https://www.wurstclient.net/serverfinder-tutorial/")))
		...
		ipBox = new EditBox(font, width / 2 - 100,
			height / 4 + 34, 200, 20, Component.empty());
		ipBox.setMaxLength(200);
		addWidget(ipBox);
		setFocused(ipBox);
		
		maxThreadsBox = new EditBox(font, width / 2 - 32,
			height / 4 + 58, 26, 12, Component.empty());
	...
	private void addServerToList(String name, String ip)
	{
		...
		selector.updateOnlineServers(serverList);
	}
