...
public final class AuraRangeRenderer
{
		...
		float green, float blue, float innerAlpha, float outerAlpha)
	{
		...
		for(int i = 0; i <= segments; i++)
		{
			...
			buffer.vertex(matrix, (float)(center.x + cos * innerRadius),
				(float)center.y, (float)(center.z + sin * innerRadius))
				.color(red, green, blue, innerAlpha).endVertex();
			buffer.vertex(matrix, (float)(center.x + cos * outerRadius),
				(float)center.y, (float)(center.z + sin * outerRadius))
