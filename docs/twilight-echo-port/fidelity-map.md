...
### 10.3b 无法移植的桌面原生能力与游戏内替代

参考项目是一个桌面播放器，它的音频与系统集成能力建立在**操作系统**之上；本工程
是运行在 Minecraft 里的一个界面，下面这些能力**不是"没做"，而是在游戏进程里没有
对应物**。每一项都给出游戏内替代，或明确写"不做"。

| 桌面能力 | 为什么在 Minecraft 里没有 | 游戏内替代 |
|---|---|---|
| **WASAPI 独占模式输出**（低延迟 / 位完美） | 游戏的音频设备由 OpenAL / LWJGL 持有，音乐又走 `goxr3plus StreamPlayer`（Java Sound），拿不到也不该拿独占句柄 | 独占在这里没有意义（不是实时监听场景）。替代：`NeteaseMusicPlayer.setVolume()` 控音量、`seekTo(long)` 精确定位、`adjustLyricOffset(long)` / `resetLyricOffset()` 手动校准歌词同步——独占模式原本要解决的"延迟导致歌词对不上"由歌词偏移直接解决 |
| **DSD（DSF/DIFF）原生解码** | Java Sound 不支持 DSD，也没有可用的软解链路 | 替代：向数据层请求尽可能高的码率。目前 `NeteaseCloudApi.resolve()` 固定 `br=320000`；要上无损/母带档需要在请求里加 `level` 参数，属未做的增强 |
| **ASIO 驱动输出** | 同上，音频设备不归本工程管 | **不做**：不在 UI 暴露该选项，避免给出无效开关 |
| **VST3 插件链 / 均衡器** | 没有音频处理图，只有 `StreamPlayer` 的增益 | 替代：音量 + 播放模式（`cyclePlaybackMode()`：列表循环 / 单曲 / 随机）。EQ 明确不做 |
| **系统托盘常驻**（最小化到托盘继续播） | Minecraft 没有托盘概念，游戏进程本身就是"常驻" | 替代：音乐本来就跟随游戏进程；`MusicLyricsHudElement`（HUD 歌词）让界面关掉后仍能看到在播什么，`.twilight` 命令随时打开新界面 |
| **全局媒体键 / 系统 SMTC** | 注册系统级热键要碰 OS 输入栈，模组不该做 | 替代：游戏内按键绑定（`MusicPlayerHack` 开关界面）+ 界面与 HUD 上的播放控制 |
| **桌面歌词**（独立透明置顶窗，含 TTML） | **本次范围明确排除**（见 §10.3 末行与用户要求） | 替代：现有 `MusicLyricsHudElement`（游戏内 HUD 歌词）+ 沉浸页由 `AppleLyricPlayer`（AMLL）渲染的歌词。参考的 TTML / 桌面歌词只作为可选数据来源 |
| **投屏 / DLNA / SMB / 局域网播放** | 需要设备发现、UPnP 与文件共享协议栈，且会向局域网暴露本机 | **不做**：本工程只播网易云直链（`resolve()` 返回的 URL）与播放器自己支持的流 |
| **音频整文件磁盘缓存**（参考的 `ncmCache.ts`：LRU / 原子落盘 / 容量清理） | 不是不可移植，只是没做 | 未做。可移植性上没有问题（纯 Java 文件 IO），列在这里是为了说明"参考有而本工程没有"的功能项里，这一项属于**可做但未做**，与上面几项的"不可做"性质不同 |

**说明**：以上替代全部只在游戏进程内生效，不改变系统音频栈，也不注册任何系统级
钩子。
