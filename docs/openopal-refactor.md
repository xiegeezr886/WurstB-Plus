...
| 旋转模型 | `IRotationModel#tick(from,to,timeDelta)`，5 种模型 | `util/RotationSmoothing`（INSTANT/LINEAR/EASE_IN_OUT/FACTOR）+ `CombatRotationController` | ⏳ 逐轴独立 vs 按向量比例分配，待评估 |
...
2. **旋转模型对齐**：评估把 `RotationSmoothing` 的逐轴限速换成 OpenOpal 的「按向量比例分配」
   （`LinearRotationModel`：`maxYaw = speed * |Δyaw|/|Δ|`，两轴同时到位）；我们现在的实现里俯仰
   只拿到 `0.7 * maxChange`，两轴不会同时收敛。**先在文档里给出对照，再决定是否改默认行为**，
   新模型以新增枚举常量的方式引入，不动现有手感。
