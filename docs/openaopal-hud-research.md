...
- **§7 第 4 步收尾：剩下两项都核实为「不适用」，没有实现，理由如下。**
  这是「先核实再动工」第 6 次改变计划，而且这次它说的是「不要做」。
  - **`Offset scoreboard` 不适用。** 参考让模块列表<b>运行时下移</b>以避开
    **原版**记分板侧栏。本工程在架构上避免了这件事：
    `IngameHudMixin.replaceScoreboardSidebar`（第 27-36 行）在 hud2 的
    `ScoreboardHudElement` 开启时直接 {@code ci.cancel()} 掉原版侧栏——
    也就是<b>替换</b>而不是共存。于是「两个东西抢右侧同一块地方」这个前提
    在 WurstB 里不存在：要么是自绘的那个记分板（自己就是一个可摆放、
    可吸附的 hud2 元素），要么是原版侧栏（用户显式选择了它）。而模块列表
    本身也是 hud2 元素、有位置和吸附，重叠由用户摆放解决，不需要硬编码偏移。
  - **`getTotalHeight()` 已被 hud2 布局系统吸收。** 参考需要它是为了上面那套
    偏移与重排的算术；本工程的 `HudManager` 已经把尺寸交给元素自己
    （`HudManager.java:323/329` 就是转发 `HackListHUD.getWidth()/getHeight()`），
    摆放交给 `HudLayout`/`HudElementConfig`。再搬一个高度求和函数是重复。
  - 顺带看到本工程<b>已经</b>用一种更彻底的方式处理过「模块列表被别的东西
    压住」：`HudManager.migrateLegacyLayout()`（第 414-426 行）发现列表与
    logo 都开着且列表还在默认偏移 (3,3) 时，把它的默认纵向偏移改成 28 并
    存盘——一次性布局迁移，而不是每帧算偏移。

- **`IslandPriority` 故意不接，并已在类注释里写明它是死代码。**
  原本打算第一个接 `MusicIslandHudElement`，核对后结论是<b>不该接</b>：
  那个岛的竞争面只有一个布尔量
  `boolean musicVisible = playing || loading`（`MusicIslandHudElement.java:185`），
  内容由 `compactText()` 三选一（歌名 / 加载中 / `mc.getFps() + " FPS"` 兜底），
  即<b>一个主状态 + 一个严格兜底</b>。优先级栈的价值来自「多个互斥主张者抢
  表头」，两个主张者且其中一个是另一个的兜底时，栈不会改变任何行为。
  要让它有价值就必须<b>发明</b>新的主张者（低血量警告、通知接管岛面……），
...
**UI 已补上**：`HudSettingsPanel`（新类）显示在编辑器右下角、工具栏之上；
悬停到有设置的元素就把面板切过去，光标移进面板后不再切换。最后这条是必须的
——否则鼠标一挪进面板、下面的元素不再被悬停，面板当场消失，永远点不到；代价
是面板不会自己消失，这是有意的。

**编辑器原有的鼠标语义已经排满了**（左键单击开关、左键拖动吸附、右键单击循环
对齐、右键拖动自由移动、中键重置缩放、滚轮缩放），所以没有引入新按钮，而是让
面板在 `mouseClicked` 里**排在最前面**：点面板的空白处和只读行也一律吃掉，
否则会穿透下去开关或拖动底下的元素，而用户以为自己点的是设置行。

**面板支持的可交互类型只有两种**：`CheckboxSetting`（点击切换）与
`EnumSetting`（点击循环）。其余（滑条、颜色、物品列表、文本框）**照常列出但
只读**、用灰色显示当前值——悄悄不显示它们比显示成只读更糟，用户会以为设置丢了。
滑条拖拽与颜色选择器仍是后续工作。

**一个顺序上的坑**：面板矩形是上一次 `render()` 记下来的，而 render 发生在输入
处理之后；第一次点到面板那一帧 `rows` 可能还是空的，所以 `mouseClicked` 在
`rows` 为空时返回 true（吃掉但不改任何东西），而不是去索引越界。

**测试**：`HudElementSettingsTest` 6 个（注册表契约：小写键、大小写撞名即报错、
不可外部修改、纯勾选框序列化成裸布尔量）；`HudSettingsPanelTest` 11 个
（`rowIndexAt` 的上下边界与半开区间、越界返回 -1、空面板，以及「面板没显示时
绝不能吃掉点击」——这条错了整个编辑器都点不动）。
